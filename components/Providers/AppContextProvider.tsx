...
export function AppContextProvider({ children }: { children: React.ReactNode }): React.ReactNode {
    const [burgerStore] = useState(() => new BurgerStore());
    const [directoriesStore] = useState(() => new DirectoriesStore());

    return <Context.Provider value={{ burgerStore, directoriesStore }}>{children}</Context.Provider>;
}
